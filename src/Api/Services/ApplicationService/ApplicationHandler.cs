    ...
    : IApplicationHandler
{
    ...
    private readonly IApprovalProcessor _approvalProcessor = approvalProcessor;
    ...
    {
        var applications = await _applicationService.GetApplicationsBySchool(schoolId);
        var responses = applications.Pagination(currentPage, pageSize)
            .Select(x => _mapper.Map<ApplicationResponse>(x))
            .ToList();
    ...
    {
        var applications = await _applicationService.GetApplicationsByDriver(driverId);
    ...
    {
        var application = await _approvalProcessor.CreateApplication(driverId, schoolId);
    ...
    private async Task<ApplicationResponse> MapToResponse(DriverApprovalRequest request)
    {
        ...
        var response = _mapper.Map<ApplicationResponse>(request);
        foreach (var file in request.VehicleImages)
        {
            var preSignDownload = await _fileUploadService.GeneratePreSignedDownloadUrlAsync(file.Key);
        ...
        {
            response.DriverRequestStatusHistoryResponse.Add(_mapper.Map<DriverRequestStatusHistoryResponse>(history));
        }
